...
#[AsCommand(
    name: 'enabel:user:promote',
    description: 'Promote a user as admin',
)]
class PromoteCommand extends Command
{
     ...
     * @var string
     */
    protected static $defaultName = 'enabel:user:promote';
    ...
    protected function configure(): void
    {
        $this
            ...
            ->addOption(
                ...
                'If set, the user is promoted as an super administrator'
            );
    }
    ...
    protected function interact(InputInterface $input, OutputInterface $output): void
    {
        ...
        $this->io->title('User promote interactive wizard');
        $this->io->text([
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $stopwatch->start('user-promote-command');
        ...
        $this->io->success(
            sprintf(
                '%s (%s) was successfully promoted: %s',
                $user->getDisplayName(),
        ...
        $event = $stopwatch->stop('user-promote-command');
        if ($output->isVerbose()) {
            $this->io->comment(
                sprintf(
                    'Promote user id: %d / Elapsed time: %.2f ms / Consumed memory: %.2f MB',
                    $user->getId(),
                    $event->getDuration(),
    ...
    {
        return <<<'HELP'
The <info>%command.name%</info> command promote a user as a admin:

  <info>php %command.full_name%</info> <comment>email</comment>
  
By default the command promote as administrator user (ROLE_ADMIN)
To promote as super administrator user (ROLE_SUPER_ADMIN), 
